        ...
        {getVisiblePageNumbers().map((pageNum, index) => (
          <PaginationItem key={index}>
            {typeof pageNum === 'string' ? (
              <PaginationEllipsis className={isMobile ? "w-6 h-6" : ""} style={{ color: 'var(--text-secondary)' }} />
            ) : (
              <PaginationLink
                isActive={paginationInfo.page === pageNum}
                onClick={() => onPageChange(Number(pageNum))}
                className={`cursor-pointer ${isMobile ? "min-w-[32px] h-8 text-sm" : ""} hover:bg-[var(--bg-muted)] transition-colors`}
                style={{ 
                  borderColor: 'var(--border)', 
                  color: paginationInfo.page === pageNum ? 'var(--text-on-primary)' : 'var(--text-primary)',
                  backgroundColor: paginationInfo.page === pageNum ? 'var(--primary)' : 'var(--bg-primary)'
                }}
              >
                {pageNum}
              </PaginationLink>
